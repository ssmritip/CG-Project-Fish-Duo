...
void translate(GLfloat upperCurve[3][3], GLfloat lowerCurve[3][3], float tCurve1[3][3], float tCurve2[3][3]){
	...
	if(t < TANK_RIGHT && s == true){
		t = t + 0.001;
		y = 0;
		for (int i = 0; i < 3; ++i){ //transpose original curve
			for (int j = 0; j < 3; ++j) {
		...
		for (int i = 0; i < 3; ++i){ //transpose back to original curve
			for (int j = 0; j < 3; ++j) {
